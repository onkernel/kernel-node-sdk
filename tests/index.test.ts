...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['KERNEL_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: Kernel) => {
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      const client = new Kernel({ logger: logger, apiKey: 'My API Key' });
    ...
    test('warn when env var level is invalid', async () => {
      ...
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'KERNEL_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
  ...
  describe('baseUrl', () => {
    test('trailing slash', () => {
      const client = new Kernel({ baseURL: 'http://localhost:5000/custom/path/', apiKey: 'My API Key' });
    ...
    afterEach(() => {
      process.env['KERNEL_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new Kernel({ baseURL: 'https://example.com', apiKey: 'My API Key' });
  ...
  test('maxRetries option is correctly set', () => {
    const client = new Kernel({ maxRetries: 4, apiKey: 'My API Key' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new Kernel({ apiKey: 'My API Key' });
  ...
  test('with environment variable arguments', () => {
    ...
    process.env['KERNEL_API_KEY'] = 'My API Key';
    const client = new Kernel();
  ...
  test('with overridden environment variable arguments', () => {
    // set options via env var
    process.env['KERNEL_API_KEY'] = 'another My API Key';
    const client = new Kernel({ apiKey: 'My API Key' });
    expect(client.apiKey).toBe('My API Key');
  });
});
...
describe('retries', () => {
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new Kernel({ apiKey: 'My API Key', fetch: testFetch });
